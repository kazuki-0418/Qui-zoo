...
const getUsers = async (_, res) => {
    const users = await user_models_1.default.getAllUsers();
    if (users.length === 0) {
...
const addNewUser = async (req, res) => {
    ...
    const userFound = await user_models_1.default.getUserByEmail(email);
    if (userFound) {
    ...
    const user = await user_models_1.default.addUser(newUser);
    if (!user) {
...
const getUserById = async (req, res) => {
    ...
    const user = await user_models_1.default.getUserById(req.params.id);
    if (!user) {
...
const updateUserById = async (req, res) => {
    const userFound = await user_models_1.default.getUserById(req.params.id);
    if (!userFound) {
    ...
    const updatedUser = await user_models_1.default.updateUser(userFound.id, newUser);
    if (!updatedUser) {
        res.status(500).json({ message: "Unable to update user" });
        return;
    }
...
const deleteUser = async (req, res) => {
    ...
    const user = await user_models_1.default.deleteUser(userFound.id);
    if (!user) {
        res.status(500).json({ message: "Unable to delete user" });
        return;
    }
...
const logUserIn = async (req, res) => {
    ...
    const userLogin = await user_models_1.default.userLogin(email, password);
    if (!userLogin) {
